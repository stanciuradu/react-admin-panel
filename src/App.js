  ...
  render() {
    return(
      ...
      <div className="app" style={{color: this.state.color}}>
        <Header/>
        <UserAddForm submitAddForm={(event, name, email, wage, src, isGoldClient) => this.submitAddForm(event, name, email, wage, src, isGoldClient)}/>
        ...
        <div className='container'>
          ...
          <input id='color' type='color' onChange={(event)=>this.handleColorChange(event)}/>
          {/* butoane pentru evenimentul de onClick */}
          <button className='button' type='button' onClick={()=>this.handleClickUsers()}>Afiseaza useri</button>
          <button className='button' type='button' onClick={()=>this.handleClickPosts()}>Afiseaza postari</button>
        </div>
      </div>
    );
  }
